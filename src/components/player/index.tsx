// import Image from 'next/image';

// import { cn } from '@/lib/utils';

// type PlayerProps = {
//   className?: string;
// };

// const Player = ({ className }: PlayerProps) => {
//   return (
//     <div className={cn('aspect-video', className)}>
//       <Image
//         fill
//         className='object-cover'
//         alt='player'
//         src='http://localhost:9001/api/v1/download-shared-object/aHR0cDovLzEyNy4wLjAuMTo5MDAwL3lvdXR1YmUtY2xvbmUvdmlkZW8vU25pcGFzdGVfMjAyNi0wMi0yOF8xNS01OS0yNy5wbmc_'
//       />
//     </div>
//   );
// };

// const FullPlayer = () => (
//   <div className='block h-[56vw] max-h-[calc(100vh-169px)] min-h-60 bg-black lg:hidden'>
//     full player
//   </div>
// );

// const MiniPlayer = () => (
//   <div className='hidden aspect-video rounded-lg bg-black lg:block'>mini player</div>
// );

// export { MiniPlayer, FullPlayer, Player };

'use client';

import '@videojs/react/video/skin.css';
import { cn } from '@/lib/utils';
import { createPlayer, videoFeatures } from '@videojs/react';
import { VideoSkin, Video } from '@videojs/react/video';

const Player1 = createPlayer({ features: videoFeatures });

interface MyPlayerProps {
  src: string;
}

const MyPlayer = ({ src }: MyPlayerProps) => {
  return (
    <Player1.Provider>
      <VideoSkin>
        <Video src={src} playsInline autoPlay />
      </VideoSkin>
    </Player1.Provider>
  );
};

const Player = ({ className }) => (
  <div className={cn('aspect-video', className)}>
    <MyPlayer src='https://stream.mux.com/uqk9VgJzJn009eT01UPDCUXmWK3H1Ar5f29021NGhkFlPM.m3u8' />
  </div>
);

export { Player };
